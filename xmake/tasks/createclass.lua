...
on_run(function()
    ...
    local files = {
        { TargetPath = path.join("Include/VoxEngine", classPath) .. ".hpp", Template = headerTemplate },
        { TargetPath = path.join("Include/VoxEngine", classPath) .. ".inl", Template = inlineTemplate }
    }

    if not option.get("nocpp") then
        table.insert(files, { TargetPath = path.join("Source", "VoxEngine", classPath) .. ".cpp", Template = sourceTemplate })
    end
    ...
    local replacements = {
        ...
        HEADER_GUARD = "VOX_" .. classPath:gsub("[/\\]", "_"):upper() .. "_HPP"
    }
...
#include <VoxEngine/%CLASS_PATH%.inl>
...
#include <VoxEngine/%CLASS_PATH%.hpp>
